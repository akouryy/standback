...
| `g/正規表現/置換文字列/` | 全置換(RubyのString#gsub)。replace部分に\1などを書いてもただの1になる。正しくは`%1`(後述)。 |
| `m/正規表現/真ブロック/偽ブロック/` | 条件分岐(正規表現がaにマッチするかどうか) |
| `q/` | `a`をエラー出力しプログラムを直ちに異常終了する(ただし`a`が空文字列の場合何も出力せず正常終了する) |
| `s/正規表現/置換文字列/` | 置換(String#sub) |
| `t/置換前文字列/置換後文字列/` | 全置換(String#tr) |
| `w/正規表現/ブロック/` | マッチする間繰り返し(while) |
